...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync('reports/seo-audit-[domain]-[date].docx', buffer);
  console.log('DOCX written');
});
```

Write the report into a `reports/` directory in the current project (create it if missing), or into the
session scratchpad directory if the project shouldn't be touched. Use a filename like
`seo-audit-example-com-2026-09-07.docx` (domain with hyphens, ISO date).

### Validate and convert to PDF

Use the `pdf` and `docx` skills available in this environment if they are listed, or LibreOffice if it
is installed:

```bash
soffice --headless --convert-to pdf reports/seo-audit-[domain]-[date].docx --outdir reports/
```

If neither is available, deliver the DOCX alone and say so — do not claim a PDF was produced.

### Deliver to the user

Use the `SendUserFile` tool to surface both files, and reference them by their repo-relative path as
markdown links so they are clickable in the terminal.
